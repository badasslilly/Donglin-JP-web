/** @format */
'use client';

import Link from 'next/link';
import clsx from 'clsx';
import { shippori } from '@/styles/fonts/fonts';
import LanguageSwitcher from '@/components/LanguageSwitcher';
import type { NavItem, Locale } from '@/lib/strapi';

const LOCALES = new Set(['ja', 'en']);

function normalizeInternalHref(href: string) {
  if (!href) return '/';
  if (/^https?:\/\//i.test(href)) return href;
  let path = href.startsWith('/') ? href : `/${href}`;
  // strip any leading locale segment (/ja/... or /en/...)
  const segs = path.split('/');
  if (LOCALES.has(segs[1])) {
    segs.splice(1, 1);
    path = segs.join('/') || '/';
  }
  return path;
}

// Prefix internal path with locale
function withLocale(href: string, locale: Locale) {
  if (!href) return `/${locale}`;
  if (/^https?:\/\//i.test(href)) return href;
  const clean = normalizeInternalHref(href);
  if (clean.startsWith('/ja/') || clean.startsWith('/en/')) return clean;
  return `/${locale}${clean}`.replace(/\/{2,}/g, '/');
}

type Props = {
  locale: Locale;
  items: NavItem[];     // ← pass from server (e.g., global.nav_items)
  className?: string;
};

export default function HomeBottomNav({ className, locale, items }: Props) {
  if (!items?.length) return null;

  const ScrollBlock = (
    <div className="flex items-center gap-2 text-base">
      <svg width="14" height="40" viewBox="0 0 14 40" aria-hidden>
        <path d="M7 0v34" stroke="currentColor" strokeWidth="1.5" />
        <path d="M12 29 7 34 2 29" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
      <span>Scroll</span>
    </div>
  );
  const LangBlock = <LanguageSwitcher />;

  return (
    <nav
      aria-label="Home navigation"
      className={clsx('pointer-events-none flex w-full flex-wrap items-center px-8 py-6', shippori.className, className)}
    >
      {/* 700-999px */}
      <div className="pointer-events-auto hidden w-full items-center justify-between gap-4 min-[700px]:flex max-[999px]:flex min-[1019px]:hidden">
        {ScrollBlock}
        {LangBlock}
      </div>

      {/* ≥1000px */}
      <div className="pointer-events-auto hidden w-full items-center min-[1020px]:flex">
        {ScrollBlock}
        <div className="ml-auto flex items-center gap-8">
          <ul className="flex gap-8 text-base ">
            {items.map((n) => {
              const href = n.isExternal ? n.href : withLocale(n.href, locale);
              const isExternal = n.isExternal || /^https?:\/\//i.test(href);
              return (
                <li key={n.href}>
                  {isExternal ? (
                    <a href={href} target="_blank" rel="noopener" className="hover:opacity-60">
                      {n.label}
                    </a>
                  ) : (
                    // ✅ allow Next to prefetch visible links
                    <Link href={href} className="hover:opacity-60" prefetch>
                      {n.label}
                    </Link>
                  )}
                </li>
              );
            })}
          </ul>
          {LangBlock}
        </div>
      </div>
    </nav>
  );
}

