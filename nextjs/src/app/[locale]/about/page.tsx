...
import Table, { HistoryGroup, HistoryRow } from '@/components/ui/Table'
      ...
      <article className='mx-auto max-w-5xl space-y-14 px-4 pb-14'>
        {(data.content ?? []).map((b: any, i: number) => {
          ...
          return (
            <section
              key={i}
              className='relative mt-14 rounded-lg border border-[#e5e2d3] bg-[#f6f3e7]/60 p-5 md:p-6 lg:p-12'
            >
              {/* bookmark (desktop) */}
              {b.headline && (
              ...
              {b.image?.url && (
                <Image
                  ...
                  width={340}
                  height={460}
                  className={`absolute top-6 hidden rounded-md md:block ${
      ...
      {groups.length > 0 && (
        <section className='mx-auto max-w-5xl px-4'>
          <SectionHeading
            title={
              locale === 'en'
                ? 'The History of Donglin Monastery'
                : '東林寺寺史'
            }
          />
